...
function check_input(){
	if(!document.fqa.writer.value)
	{
		alert("이름을 입력하세요");
		document.fqa.writer.focus();
		return;
	}
	if(!document.fqa.title.value)
	{
		alert("제목을 입력하세요");
		document.fqa.title.focus();
		return;
	}
	if(!document.fqa.content.value)
	{
		alert("내용을 입력하세요");
		document.fqa.content.focus();
		return;
	}
	document.fqa.submit();
}
...
<? include "../lib/admin_notice.php";?>
  <div id="notice_main">
    <div id="notice_main_title">자주묻는 질문</div>
		<?php if($mode == "modify"){ ?>
		<form name="notice" method="post" action="../db/fqa_insert.php?mode=modify&&num=<?=$num?>" enctype="multipart/form-data">
		<?php } else { ?>
	<form name="fqa" method="post" action="../db/fqa_insert.php" enctype="multipart/form-data">
	<?php } ?>
    <div id="write_box">
      <div id="title">이름</div>
